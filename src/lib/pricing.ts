...
export function calculateRentalPrice(input: CalculationInput): PricingBreakdown {
    const {
        vehicle,
        pickupDate,
        dropDate,
    ...
    const actualEndTime = actualDropDate ? new Date(actualDropDate).getTime() : scheduledEndTime;
    ...
    // 6. Tax
    const taxableAmount = baseRate + extraKmCharge + lateReturnCharge + fuelRefillCharge + damages;
