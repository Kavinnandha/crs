        ...
        <div className="space-y-6">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                <PageHeader
                    ...
                    breadcrumb={["Dashboard", "Vehicles", vehicle.brand + " " + vehicle.model]}
                />
                    ...
                    <CardContent className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                        <div className="space-y-4">
                            <div>
                                ...
                                <p className="font-mono text-lg">{vehicle.registrationNumber}</p>
                            </div>
                            <div>
                        ...
                        <div className="space-y-4">
                            <div>
                                <h4 className="text-sm font-medium text-muted-foreground mb-1">Fuel & Transmission</h4>
                                <p className="text-base">{vehicle.fuelType} • {vehicle.transmission}</p>
                            </div>
                            <div>
                        ...
                        <CardContent className="space-y-6">
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    ...
                                    <p className="text-xl font-bold">{formatCurrency(vehicle.pricePerDay)}</p>
                                </div>
                            </div>
                            <Separator />
                            <div className="flex items-center gap-4">
                            ...
                            <TableBody>
                                {vehicleBookings.map((booking: Booking) => (
                                    <TableRow key={booking.id}>
                                        <TableCell className="font-medium">{booking.customerId}</TableCell>
                                        <TableCell>{new Date(booking.pickupDate).toLocaleDateString()}</TableCell>
                                        <TableCell>{new Date(booking.dropDate).toLocaleDateString()}</TableCell>
                                        <TableCell>
