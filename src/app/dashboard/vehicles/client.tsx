...
export default function VehiclesClient({
    ...
    if (vehicleToDelete) {
      ...
      try {
        ...
        if (result && result.success) {
          ...
          setDeleteOpen(false);
          setAlertMessage(
            "An unexpected error occurred (no response from server)",
          );
          setAlertOpen(true);
        }
      } catch (error) {
  ...
  const getCategoryStyle = (category: string) => {
    switch (category) {
      case "SUV":
        return "bg-violet-50 dark:bg-violet-950/30 text-violet-600 border-violet-200 dark:border-violet-800";
      case "Sedan":
        return "bg-sky-50 dark:bg-sky-950/30 text-sky-600 border-sky-200 dark:border-sky-800";
      case "Hatchback":
        return "bg-teal-50 dark:bg-teal-950/30 text-teal-600 border-teal-200 dark:border-teal-800";
      case "Luxury":
        return "bg-rose-50 dark:bg-rose-950/30 text-rose-600 border-rose-200 dark:border-rose-800";
      default:
        return "bg-gray-50 dark:bg-gray-950/30 text-gray-600 border-gray-200 dark:border-gray-800";
    }
  };
    ...
    <div className="flex flex-wrap items-center gap-2 justify-end">
      ...
      <Select value={categoryFilter} onValueChange={setCategoryFilter}>
        ...
        <SelectContent className="rounded-xl border-border shadow-lg">
          ...
          <SelectItem value="Hatchback" className="rounded-lg">
            Hatchback
          </SelectItem>
          <SelectItem value="Sedan" className="rounded-lg">
            Sedan
          </SelectItem>
          <SelectItem value="SUV" className="rounded-lg">
            SUV
          </SelectItem>
          <SelectItem value="Luxury" className="rounded-lg">
            Luxury
          </SelectItem>
        </SelectContent>
      </Select>
      ...
      {viewMode === "table" ? (
                ...
                {filteredVehicles.length === 0 ? (
                  ...
                  filteredVehicles.map((vehicle) => (
                    ...
                    >
                      ...
                      <TableCell>
                        <span
                          className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium border ${getCategoryStyle(vehicle.category)}`}
                        >
                          {vehicle.category}
                        </span>
                      </TableCell>
                      ...
                      <TableCell className="font-medium text-[#1a1d2e] dark:text-white">
                        ₹{vehicle.pricePerDay}
                      </TableCell>
                      <TableCell>
                        <DropdownMenu>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {filteredVehicles.map((vehicle) => (
            <Card
              key={vehicle.id}
              className="group relative overflow-hidden transition-all hover:shadow-lg hover:-translate-y-1 border-border bg-card rounded-3xl"
            >
              <CardContent className="p-6 flex flex-col h-full">
                <div className="flex justify-between items-start mb-4">
                  <div className="space-y-1">
                    ...
                    >
                      <h3 className="font-bold text-lg text-[#1a1d2e] dark:text-white line-clamp-1">
                        {vehicle.brand} {vehicle.model}
                      </h3>
                    </Link>
                ...
                <div className="mt-auto pt-4 border-t border-border flex items-center justify-between">
                  ...
                  >
                    <Button
                      variant="outline"
                      className="w-full h-9 text-xs font-medium rounded-xl border-slate-200 hover:bg-slate-50 dark:border-slate-700 dark:hover:bg-slate-800"
                    >
                      View Details
                    </Button>
                  </Link>
                  ...
                  <DropdownMenu>
                    <DropdownMenuTrigger asChild>
                      <Button
                        variant="ghost"
                        size="icon"
                        className="h-9 w-9 text-[#94a3b8] hover:text-[#1a1d2e] hover:bg-slate-50 dark:hover:bg-slate-800 rounded-xl shrink-0"
                      >
                        <MoreHorizontal className="h-4 w-4" />
                      </Button>
                    </DropdownMenuTrigger>
                    <DropdownMenuContent
                      align="end"
                      className="rounded-xl border-border shadow-lg p-1 w-32"
                    >
        ...
        <DialogContent className="max-w-sm rounded-2xl border-border shadow-xl">
          <DialogHeader>
            ...
            <DialogDescription className="text-[#94a3b8]">
              Are you sure you want to delete {vehicleToDelete?.brand}{" "}
