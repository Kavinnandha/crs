...
export default function ReportsClient({
  ...
  const utilizationRate =
    Math.round(
      (bookings.filter((b) => b.status === "Active").length / vehicles.length) *
        100,
    ) || 0;
            ...
            <Table>
              <TableHeader>
                <TableRow className="border-b border-slate-100 dark:border-slate-800 bg-white dark:bg-transparent">
                  <TableHead className="w-[50px] font-medium text-xs text-muted-foreground">
                    #
                  </TableHead>
              ...
              <TableBody>
                {topVehicles.map((v, i) => (
                  <TableRow
                    key={v.id}
                    className="border-b border-slate-50 dark:border-slate-800 last:border-0 hover:bg-slate-50/50 dark:hover:bg-slate-800/30"
                  >
                    ...
                    <TableCell className="text-right">
                      <Badge
                        variant="secondary"
                        className="bg-violet-50 dark:bg-violet-950/30 text-violet-700 dark:text-violet-300 hover:bg-violet-100 dark:hover:bg-violet-950/50"
                      >
                        {v.bookingCount}
                      </Badge>
                    </TableCell>
                    <TableCell className="text-right font-medium text-slate-700 dark:text-slate-200 pr-4">
          ...
          <CardHeader className="flex flex-row items-center justify-between pb-2 bg-slate-50/50 dark:bg-slate-800/50">
            <div className="space-y-1">
              <CardTitle className="text-base font-semibold flex items-center gap-2">
                ...
                <Badge
                  variant="outline"
                  className="text-emerald-600 bg-emerald-50 dark:bg-emerald-950/30 border-emerald-200 dark:border-emerald-800"
                >
                  {idleVehicles.length}
                </Badge>
              </CardTitle>
          ...
          <CardContent className="p-4 grid gap-3">
            {idleVehicles.length === 0 ? (
              ...
              idleVehicles.map((v) => (
                <div
                  key={v.id}
                  className="flex items-center justify-between p-3 rounded-xl border border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900 hover:border-violet-200 dark:hover:border-violet-800 transition-colors group"
                >
