...
export default async function CustomerDetailsPage({
  ...
  const vehicleMap = new Map<string, Vehicle>(
    allVehicles.map((v: Vehicle) => [v.id, v]),
  );
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <Card className="lg:col-span-1 shadow-sm border-[#E8E5F0] dark:border-slate-800 overflow-hidden">
          <div className="bg-linear-to-br from-indigo-50 to-purple-50 dark:from-slate-800 dark:to-slate-900 p-6 flex flex-col items-center border-b border-[#E8E5F0] dark:border-slate-800">
            ...
            <h2 className="font-semibold text-xl text-[#1a1d2e] dark:text-white">
              {customer.name}
            </h2>
            <div className="mt-2">
              <StatusBadge
          ...
          <CardContent className="p-6 space-y-4">
            <div className="flex items-center gap-3 p-3 rounded-lg bg-[#F8F9FC] dark:bg-slate-800 border border-[#E8E5F0] dark:border-slate-700">
              ...
              <span className="text-sm truncate font-medium text-[#1a1d2e] dark:text-slate-200">
                {customer.email}
              </span>
            </div>
            <div className="flex items-center gap-3 p-3 rounded-lg bg-[#F8F9FC] dark:bg-slate-800 border border-[#E8E5F0] dark:border-slate-700">
              <Phone className="h-4 w-4 text-[#7C3AED] shrink-0" />
              <span className="text-sm font-medium text-[#1a1d2e] dark:text-slate-200">
                {customer.phone}
              </span>
            </div>
            <div className="flex items-center gap-3 p-3 rounded-lg bg-[#F8F9FC] dark:bg-slate-800 border border-[#E8E5F0] dark:border-slate-700">
              <MapPin className="h-4 w-4 text-[#7C3AED] shrink-0" />
              <span className="text-sm font-medium text-[#1a1d2e] dark:text-slate-200 truncate">
                {customer.address || "No address provided"}
              </span>
            </div>
            <div className="flex items-center gap-3 p-3 rounded-lg bg-[#F8F9FC] dark:bg-slate-800 border border-[#E8E5F0] dark:border-slate-700">
              <CreditCard className="h-4 w-4 text-[#7C3AED] shrink-0" />
              <span className="text-sm font-mono font-medium text-[#1a1d2e] dark:text-slate-200">
              ...
              <Table>
                <TableHeader>
                  <TableRow className="border-[#E8E5F0] dark:border-slate-800 bg-[#F8F9FC] dark:bg-slate-800/50 hover:bg-[#F8F9FC] dark:hover:bg-slate-800/50">
                    ...
                    <TableHead className="text-right text-[#64748B]">
                      Amount
                    </TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {customerBookings.map((booking: Booking) => {
                    ...
                    return (
                      <TableRow
                        key={booking.id}
                        className="border-[#E8E5F0] dark:border-slate-800 hover:bg-[#F8F9FC] dark:hover:bg-slate-800/30"
                      >
                        <TableCell>
                          <div>
